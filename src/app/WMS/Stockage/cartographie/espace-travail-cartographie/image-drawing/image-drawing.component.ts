...
export class ImageDrawingComponent implements OnInit, OnChanges {
  ...
  public saveImage() {
    if (
      !this.forceSizeExport ||
      (this.forceSizeExport && this.width && this.height)
    ) {
      const canvasScaledElement: HTMLCanvasElement = document.createElement(
        "canvas"
      );
      ...
      new Observable<fabric.Canvas>(observer => {
        if (this.imageUsed) {
          if (this.forceSizeExport) {
            ...
            this.imageUsed.cloneAsImage((imageCloned: { scaleToWidth: (arg0: number, arg1: boolean) => void; scaleToHeight: (arg0: number, arg1: boolean) => void; }) => {
              ...
              canvasScaled.setBackgroundImage(
                ...
                (img: HTMLImageElement) => {
                  if (!img) {
                    observer.error(
                      new Error(
                        "Impossible to draw the image on the temporary canvas"
                      )
                    );
                  }
        ...
        .pipe(
          switchMap(() => {
            ...
            if (this.canvas.getObjects().length > 0) {
              ...
              this.canvas
                .getObjects()
                .forEach((originalObject: fabric.Object, i: number) => {
                    ...
                    switchMap(() => {
                      return new Observable<fabric.Canvas>(observerObject => {
                        originalObject.clone((clonedObject: fabric.Object) => {
                          clonedObject.set(
                            "left",
                            originalObject.left * ratioX
                          );
                          clonedObject.set("top", originalObject.top * ratioY);
                          clonedObject.scaleToWidth(
                            originalObject.width * ratioX
                          );
                          clonedObject.scaleToHeight(
                            originalObject.height * ratioY
                          );
  ...
  public getTextTranslated(name: string): string {
    ...
    if (this.i18nUser) {
      try {
        const str = name
          .split(".")
          .reduce((o, i) => o[i], this.i18nUser as any);
        if (str) {
          strOk = str;
        }
      } catch (e) {
  ...
  private importPhotoFromSrc(src: string) {
    ...
    imgEl.onerror = () => {
      ...
      if (isFirstTry) {
        ...
        this.hasError = true;
        this.errorMessage = this.getTextTranslated("loadError").replace(
          "%@",
          this.src as string
        );
      }
    ...
    imgEl.onload = () => {
      ...
      this.imageUsed.cloneAsImage((image: { scaleToWidth: (arg0: number, arg1: boolean) => void; scaleToHeight: (arg0: number, arg1: boolean) => void; getScaledWidth: () => any; getScaledHeight: () => any; }) => {
